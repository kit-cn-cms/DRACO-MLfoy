...
result_dir = "/ceph/vanderlinden/DRACO-MLfoy/WiggleStudies/results/singleWiggle/"
...
wiggle_factor = 0.2
input_features = dnn_aachen.train_variables
...
for variable in input_features:
    print("wiggling variable "+str(variable))
    # apply some uncertainties to data
    def func(x):
        return x + np.random.normal(0,wiggle_factor)

    # wiggle data
    data_new = data.copy()
    data_new[variable] = data[variable].apply(func)
    ...
    save_dir = result_dir + "/{}".format(variable.replace("[","_").replace("]",""))
    ...
    ax0.set_title("{} wiggled by {:.3f}".format(variable, wiggle_factor), loc = "left")
...
# DIFF PLOT
xvals = np.arange(1,len(input_features)+1,1)
plt.errorbar(xvals, mean_diff, yerr = std_diff, fmt = "o")
plt.ylabel("mean of absolute difference in prediction")
plt.title("category "+str(key), loc = "right")
plt.title("wiggle factor {:.3f}".format(wiggle_factor), loc = "left")
ax = plt.gca()
ax.set_xticks(np.arange( len(input_features))+1, minor = False )
ax.set_xticklabels(input_features, rotation = 90)
plt.grid()
plt.tight_layout()
...
# ARGMAX PLOT
plt.plot( xvals, rate_of_other_argmax, "o")
