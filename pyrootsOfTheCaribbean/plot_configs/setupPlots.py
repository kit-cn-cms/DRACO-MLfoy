...
def drawHistsOnCanvas(sigHists, bkgHists, sigHists_unscaled, plotOptions, canvasName,displayname=None,logoption=False):
    if not displayname:
    ...
    if not isinstance(bkgHists, list):
        bkgHists = [bkgHists]
    if not isinstance(sigHists_unscaled, list):
        sigHists_unscaled = [sigHists_unscaled]
    ...
    if plotOptions["ratio"]:
        canvas.cd(2)
        # print("# DEBUG: setupPlots, sigHists: ", sigHists)
        line = sigHists_unscaled[0].Clone()
        line.Divide(sigHists_unscaled[0])
        line.GetYaxis().SetRangeUser(0.,1.1)
        ...
        line.DrawCopy("histo")
        # ratio plots
        for sigHist in sigHists_unscaled:
