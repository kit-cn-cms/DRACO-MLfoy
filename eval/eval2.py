...
def find_n_best(event,n):
    ...
    minchi = np.zeros(n+1) + 999999
    minchi[0]=0
    for i in range(n):
        globals()["thadbest" + str(i)] = ROOT.TLorentzVector()
        globals()["tlepbest" + str(i)] = ROOT.TLorentzVector()
                ...
                for m in range(njets):
                    ...
                    for i in range(n):
                        ...
                        if(chi2>minchi[i] and chi2<minchi[i+1]):
                            #set value into minchi[i+1] and move all other entries one index up
                            for ind in range(n-1,i+1,-1):
                                globals()["tlepbest" + str(ind)]    = globals()["tlepbest" + str(ind-1)]
                                globals()["thadbest" + str(ind)]    = globals()["thadbest" + str(ind-1)]
                                minchi[ind]     = minchi[ind-1]
                                bestcombi[ind-1][:] = bestcombi[ind-2][:]
                            globals()["tlepbest" + str(i+1)]    = tlep
...
for event in chain:
    ...
    for j in range(10):
        for index2 in pepec:
            globals()["TopHad_B" + "_" + index2] = np.append(globals()["TopHad_B" + "_" + index2], globals()[index2][int(presel[j][0])])
            globals()["TopLep_B" + "_" + index2] = np.append(globals()["TopLep_B" + "_" + index2], globals()[index2][int(presel[j][1])])
            globals()["TopHad_Q1" + "_" + index2] = np.append(globals()["TopHad_Q1" + "_" + index2], globals()[index2][int(presel[j][2])])
            globals()["TopHad_Q2" + "_" + index2] = np.append(globals()["TopHad_Q2" + "_" + index2], globals()[index2][int(presel[j][3])])
...
leg_had.Draw()
c1.SaveAs("/nfs/dust/cms/user/jdriesch/results/10_test_eff_1p.pdf")
...
c2.SaveAs("/nfs/dust/cms/user/jdriesch/results/10_toptest_1p.pdf")
