...
def normalize_vec(vec,inputdir):
    unnormed_vec = vec
    ...
    for ind in variables:
        cc+=1
        vec[cc] = (unnormed_vec[cc] - df_norms[ind][0])/df_norms[ind][1]
    return vec
...
for event in chain:
    ...
    for index in ["TopHad","TopLep", "WHad", "WLep"]:
        for index2 in ["Pt", "Eta", "Phi", "M", "logM"]:
            ...
            globals()["reco_" + index + "_" + index2] = 0


    ttbar_phi     = np.array([])
    ttbar_pt_div_ht_p_met     = np.array([])

    test = np.zeros(len(variables))
    best_prediction = -1
    asdf = 0
...
#=========================neues start============
                    ...
                    for index2 in pepec:
                        globals()["TopHad_B" + "_" + index2] = globals()[index2][j]
                        globals()["TopLep_B" + "_" + index2] = globals()[index2][k]
                        globals()["TopHad_Q1" + "_" + index2] = globals()[index2][l]
                        globals()["TopHad_Q2" + "_" + index2] = globals()[index2][m]
                    ...
                    for index in ["TopHad","TopLep", "WHad","WLep"]:
                        globals()["reco_" + index +"_Pt"] = locals()["reco_" + index + "_4vec"].Pt()
                        globals()["reco_" + index + "_Eta"] = locals()["reco_" + index + "_4vec"].Eta()
                        globals()["reco_" + index + "_Phi"] = locals()["reco_" + index + "_4vec"].Phi()
                        globals()["reco_" + index + "_M"] = locals()["reco_" + index + "_4vec"].M()
                        globals()["reco_" + index + "_logM"] = log( locals()["reco_" + index + "_4vec"].M())

                    ttbar_phi     = correct_phi(reco_TopHad_4vec.Phi()-reco_TopLep_4vec.Phi())
                    ttbar_pt_div_ht_p_met     = (reco_TopHad_4vec.Pt() + reco_TopLep_4vec.Pt())/(Ht + Evt_MET_Pt + lepton_4vec.Pt())

                    cj=-1
                    for var in variables:
                        cj+=1
                        if var[:4] in ["Muon", "Elec"]:
                            test[cj] = locals()[var[:-3]]
                        else:
                            test[cj] = globals()[var]
                    # print test

                    vec_normed = normalize_vec(test,inPath)
                    # print vec_normed, vec_normed.shape

                    prediction = model.predict(np.array([vec_normed]))

                    if(prediction>best_prediction):
                        best_prediction = prediction
                        tlep = reco_TopLep_4vec
                        thad = reco_TopHad_4vec

                    asdf+=1
                    if asdf%80==0: print "lol"
...
#=============================00altes ende=================
    ...
    if(i%100==0):
        print i, '/', nchain/2., '=', 200.*i/nchain,'%', "      , eff: ", 1.*counter/eventcounter, "       , thad-eff: ", 1.*rcounter/eventcounter, "          , tlep-eff: ", 1.*lcounter/eventcounter
...
c1.SaveAs("/nfs/dust/cms/user/jdriesch/results/results/" + options.inputDir + "_" + options.variableSelection+ "_tlep" + options.percentage + "p_jetpt_cut.pdf")
...
leg_had.Draw()
c2.SaveAs("/nfs/dust/cms/user/jdriesch/results/results/" + options.inputDir + "_" + options.variableSelection + "_thad" + options.percentage + "p_jetpt_cut.pdf")
...
c3.SaveAs("/nfs/dust/cms/user/jdriesch/results/results/" + options.inputDir + "_" + options.variableSelection + "_reko_qual_" + options.percentage + "p_jetpt_cut.pdf")
