...
class CNN():
    ...
    def build_default_model(self):
        ''' build default straight forward CNN from architecture dictionary '''
    ...
    def save_model(self, argv, execute_dir):
        ...
        # save information for binary CNN
...
def loadCNN(inputDirectory, outputDirectory, binary = False, signal = None, binary_target = None, total_weight_expr = 'x.Weight_XS * x.Weight_CSV * x.Weight_GEN_nom'):
    ...
    if not os.path.exists(configFile):
        sys.exit("config needed to load trained CNN not found\n{}".format(configFile))
    ...
    cnn.load_trained_model(inputDirectory)
#    cnn.predict_event_query()
