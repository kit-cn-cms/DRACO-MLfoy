...
class DenseVariational(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self._kl_divergence_fn = _make_kl_divergence_penalty(self.kl_use_exact, weight=self.kl_weight)
  ...
  def build(self, input_shape):
    ...
    self._posterior = self.make_posterior_fn(
        last_dim * self.units,
        self.units if self.use_bias else 0,
        dtype)
    self._prior = self.make_prior_fn(
        last_dim * self.units,
        self.units if self.use_bias else 0,
        dtype)
  ...
  def get_config(self):
    ...
    for function_key in function_keys:
      ...
      if function is None:
        ...
        function_name, function_type = tfp_layers_util.serialize_function(function)
