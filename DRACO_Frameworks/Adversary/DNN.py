...
class DNN():
    ...
    def predict_event_query(self, query ):
        ...
        for index, row in events.iterrows():
            ...
            for i, value in enumerate(output):
                print(str(self.event_classes[i])+" node: "+str(value))
            print("-------------------->")
    ...
    def build_default_adversary_model(self,penalty=0):
        ''' default straight forward DNN '''
        K.set_learning_phase(True)
        number_of_input_neurons     = self.data.n_input_neurons
        ...
        self.class_model.compile(loss = self.architecture["loss_function"],optimizer=self.optimizer,metrics = self.eval_metrics)
        self.class_model.summary()

        self.adv_model.compile(loss=[make_loss_Adv(c=1.0)], optimizer=self.optimizer,
        ...
        self.class_adv_model.compile(loss=[make_loss_Class(c=1.0),make_loss_Adv(c=-penalty)],
        ...
        self.adv_class_model.compile(loss =[make_loss_Adv(c=1.)], optimizer=self.optimizer,
            metrics = self.eval_metrics)
    ...
    def train_adversary_model(self):
