...
class Sample:
    def __init__(self, path, label, normalization_weight = 1., isTrainSample = True, signalSample = False):
        ...
        self.count_adversary_samples = 0
...
class DataFrame(object):
    def __init__(self,
        ...
        df["index_label"] = pd.Series( [self.class_translation[c[:-3]] if (c.endswith('_OL')) else self.class_translation[c] for c in df["class_label"].values], index = df.index )
        #print(df["class_label"])
        #print(df["index_label"])
        #print(df["generator_flag"])
        ...
        self.n_output_neurons = len(self.classes)-input_samples.addtional_samples
