...
nn1_pred, nn1_pred_std = ann_calc_mean_std(model=dnn, input_dir=input_dir_1, n_NNs=n_iterations)
nn2_pred, nn2_pred_std = ann_calc_mean_std(model=dnn_qt, input_dir=input_dir_2, n_NNs=n_iterations)

# Comparison multiAnns (with and without QT)
plot_correlation_two_NNs(nn1_pred, nn2_pred, nn1_pred_std, nn2_pred_std, "(Multi-ANN)"+ r'$^{\rm{D}}_{1 \times 150}$', "(Multi-ANN)" +r'$^{\rm{D,T}}_{1 \times 150}$', output_dir, "Multi_ANN_comparison", privateWork=options.isPrivateWork())