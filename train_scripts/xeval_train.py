...
input_samples.addSample("tthf"+naming,  label = "tthf", normalization_weight = nom_weight)
input_samples.addSample("ttmb"+naming,  label = "ttmb", normalization_weight = nom_weight)
...
for types in dnn_type:
    # initializing DNN training class
    dnns.append(DNN.DNN(
        save_path       = outputdir+types[0],
...
if options.xEval:
    if options.plot:
        ...
        DNN.plot_crossEval(dnn_even = dnns[0], dnn_odd = dnns[1], plotdir = outputdir, log = options.log, signal_class = options.signal_class, privateWork = options.privateWork)
