# python evaluating_models_only_kinematics.py -i "/local/scratch/ssd/ycung/new_h5_files_2017_v2" -o comparison_only_kinematics -c ge4j_ge3t -v new_variableset_25_v2 --binary -S ttH -q -P --restorefitdir /home/ycung/Desktop/DRACO-MLfoy_thesis/workdir/training_ANN_150N/QT_ANN_150N_training_0_ge4j_ge3t/fit_data.pck
# global imports
...
input_dir_1  = work_dir+"Var_only_kinematics_BNN_training_50_ge4j_ge3t"
input_dir_2  = work_dir+"Var_only_kinematics_QT_BNN_training_50_ge4j_ge3t"
input_dir_3  = work_dir+"Var_BNN_training_50_ge4j_ge3t"
input_dir_4  = work_dir+"Var_QT_BNN_training_50_ge4j_ge3t"

# get net config json
def getVariablesfromConfig(inputDirectory):
    configFile = inputDirectory+"/checkpoints/net_config.json"
    if not os.path.exists(configFile):
        sys.exit("config needed to load trained NN not found\n{}".format(configFile))
    with open(configFile) as f:
        config = f.read()
    config = json.loads(config)
    return config["trainVariables"]

bnn_kin_qt = BNN.BNN(
    save_path       = options.getOutputDir(),
    input_samples   = input_samples_binary, #changed
    category_name   = options.getCategory(),
    train_variables = getVariablesfromConfig(input_dir_2), #changed!!
    # number of epochs
    ...
    restore_fit_dir = "/home/ycung/Desktop/DRACO-MLfoy_thesis/workdir/Var_only_kinematics_QT_BNN_training_50_ge4j_ge3t/fit_data.pck", #changed!!
    sys_variation   = False,
    gen_vars        = False)
...
bnn_kin = BNN.BNN(
    ...
    input_samples   = input_samples_binary, #changed
    category_name   = options.getCategory(),
    train_variables = getVariablesfromConfig(input_dir_1), #changed!!
    # number of epochs
    train_epochs    = options.getTrainEpochs(),
    # metrics for evaluation (c.f. KERAS metrics)
    eval_metrics    = ["acc"],
...
bnn = BNN.BNN(
    ...
    input_samples   = input_samples_binary, #changed
    category_name   = options.getCategory(),
    train_variables = options.getTrainVariables(), #changed!!
    # number of epochs
    train_epochs    = options.getTrainEpochs(),
    # metrics for evaluation (c.f. KERAS metrics)
    eval_metrics    = ["acc"],
...
nn1_pred, nn1_pred_std, labels1 = bnn_kin.load_trained_model(input_dir_1, n_iterations, options.getOutputDir())
nn2_pred, nn2_pred_std, labels2 = bnn_kin_qt.load_trained_model(input_dir_2, n_iterations, options.getOutputDir())
nn3_pred, nn3_pred_std, labels3 = bnn.load_trained_model(input_dir_3, n_iterations, options.getOutputDir())
nn4_pred, nn4_pred_std, labels4 = bnn_qt.load_trained_model(input_dir_4, n_iterations, options.getOutputDir())

# Comparison with and without QT
plot_correlation_two_NNs(nn1_pred, nn2_pred, nn1_pred_std, nn2_pred_std, "(BNN)" + r'$^{\rm{DV^*}}_{1 \times 50}$', "(BNN)" + r'$^{\rm{DV^*,T}}_{1 \times 50}$', output_dir, "BNN_Var_only_kin_comparison", privateWork=options.isPrivateWork())
plot_correlation_two_NNs(nn3_pred, nn4_pred, nn3_pred_std, nn4_pred_std, "(BNN)" + r'$^{\rm{DV}}_{1 \times 50}$', "(BNN)" + r'$^{\rm{DV,T}}_{1 \times 50}$', output_dir, "BNN_Var_comparison", privateWork=options.isPrivateWork())

# Comparison BNN with different variable sets (with and without QT)
plot_correlation_two_NNs(nn1_pred, nn3_pred, nn1_pred_std, nn3_pred_std, "(BNN)" + r'$^{\rm{DV^*}}_{1 \times 50}$', "(BNN)" + r'$^{\rm{DV}}_{1 \times 50}$', output_dir, "BNN_Var_only_kin_BNN_Var_comparison", privateWork=options.isPrivateWork())
plot_correlation_two_NNs(nn2_pred, nn4_pred, nn2_pred_std, nn4_pred_std, "(BNN)" + r'$^{\rm{DV^*,T}}_{1 \times 50}$', "(BNN)" + r'$^{\rm{DV,T}}_{1 \times 50}$', output_dir, "QT_BNN_Var_only_kin_BNN_Var_comparison", privateWork=options.isPrivateWork())
