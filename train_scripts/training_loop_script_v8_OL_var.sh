...
#_v8 kl_use_exact  = use default settings default settings for initializing posterior mean (kernel + bias) (=initialize to zero) but initializing posterior std (kernel + bias) to approx. 1.
#_v8_OL = initialization modifications now also applied to OutputLayer (OL)
...
for i in "${!layers[@]}"; do
    python train_template_bnn_TEST_3_OL.py -o $output1"${layers[$i]}"_v8_OL -i /local/scratch/ssd/nshadskiy/2017_nominal -c ge4j_ge3t -v allVariables_2017_bnn -n "$name" -p --printroc --binary --signal ttH -e $epochs -q --layers ${layers[$i]}
    python train_template_bnn_TEST_3_OL.py -o $output2"${layers[$i]}"_v8_OL -i /local/scratch/ssd/nshadskiy/2017_nominal -c ge4j_ge3t -v allVariables_2017_bnn -n "$name" -p --printroc --binary --signal ttH -e $epochs --layers ${layers[$i]}
done
