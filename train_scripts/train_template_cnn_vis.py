...
def readOutFilters(l, name):
    print('#'*60)
    print('read out filters '+ name + ' training')
    text_file = open(options.getOutputDir() + '/filterOutputs_' + name + '_training' + options.getNaming()  + '.txt', "w")
...
readOutFilters(model.get_weights()[0],'before')
...
readOutFilters(model.get_weights()[0],'after')
