...
#_V20 = BNN_v20 benutzt (Flipour sollte das gl. Setting wie V5_Var haben:
...
layers=("50" "100" "200" "50,50" "50,50,50" "100,100" "150,150" "100,100,100" "150,150,150" "250" "300" "400" "500" "200,200" "250,250" "200,200,200" "250,250,250")


for i in "${!layers[@]}"; do
    python train_template_bnn_denseflipout_test.py -o $output1"${layers[$i]}"_v20 -i /local/scratch/ssd/nshadskiy/2017_nominal -c ge4j_ge3t -v allVariables_2017_bnn -n "$name" -p --printroc --binary --signal ttH -e $epochs -q --layers ${layers[$i]}
    python train_template_bnn_denseflipout_test.py -o $output2"${layers[$i]}"_v20 -i /local/scratch/ssd/nshadskiy/2017_nominal -c ge4j_ge3t -v allVariables_2017_bnn -n "$name" -p --printroc --binary --signal ttH -e $epochs --layers ${layers[$i]}
done