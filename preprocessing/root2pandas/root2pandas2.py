...
# function to get dataframe with all variables relevant for ttbar-reconstruction
    def findbest(self, df):
                ...
                for k in range(njets):
                    for l in range(njets):
                        for m in range(njets):
                            ...
                            deltar_q12   = ((Eta[m]-GenTophad_Q1_Eta[i])**2 + (correct_phi(Phi[m] - GenTophad_Q1_Phi[i]))**2)**0.5
                            deltar_q22   = ((Eta[l]-GenTophad_Q2_Eta[i])**2 + (correct_phi(Phi[l] - Gentophad_Q2_Phi[i]))**2)**0.5


                            betterlm = (deltar_q1 + deltar_q2 < deltar_q12 + deltar_q22)
                            if betterlm:  total_deltar = deltar_bhad + deltar_blep + deltar_q1 + deltar_q2
                            if not betterlm: total_deltar = deltar_bhad + deltar_blep + deltar_q12 + deltar_q22
                            ...
                            if(total_deltar<minr):
                                if self.Scale :
                                    if randrange(2):
                                        ...
                                        bkg = np.append(bkg, [[best_comb[0],best_comb[1],best_comb[3],best_comb[2]]], axis=0)
                                minr = total_deltar
                                if betterlm:
                                    best_comb = [j,k,l,m]
                                    minr_q1 = deltar_q1
                            ...
                            if (total_deltar >= minr and self.Scale):
                                if randrange(2):
                                    ...
                                    bkg = np.append(bkg, [[j,k,m,l]], axis = 0)
        ...
        plt.ylabel("Anteil akzeptierter Ereignisse")
        plt.grid()
